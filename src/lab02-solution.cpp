 ...
 * @todo Color data (RGB), just append another 3 vectors for RGB data (float, [0,1])
 */
static glm::vec3 vertices[6] =
{
  { -0.6f, -0.4f, 0.f },
  {  0.6f, -0.4f, 0.f },
  {   0.f,  0.6f, 0.f },
...
int main(void)
{
  ...
  std::string vertex_shader_text = readFile("shaders/simple-solution.vert");
  std::string fragment_shader_text = readFile("shaders/simple-solution.frag");
...
Comment from https://www.cs.utexas.edu/~fussell/courses/cs384g-fall2013/lectures/lectureXX-OpenGL.pdf
  ...
  */
  ...
  glVertexAttribPointer(1,             // layout index for position in shader
                        ...
                        GL_FALSE,      // do not normalize values
                        0,             // stride in bytes
                        (GLvoid*)(3*sizeof(glm::vec3))// offset in array for first component
    );
  glEnableVertexAttribArray(1); // enable vertex attribute with layout index 0
