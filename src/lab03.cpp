...
static constexpr int MESH_WIDTH = 1024;
static constexpr int MESH_HEIGHT = 1024;
...
int main(int argc, char** argv)
{
  ...
  // set screen size
  window = glfwCreateWindow(640, 480, "Attractor", NULL, NULL);
  if (!window)
  {
  ...
  */
  ...
  std::string vertex_shader_text   = readFile("shaders/particles.vert");
  std::string geom_shader_text     = readFile("shaders/particles.geom");
  std::string fragment_shader_text = readFile("shaders/particles.frag");
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    glm::mat4 projection = glm::ortho( -0.5f*frame_width, 0.5f*frame_width,
                                       ...
                                       1.f, -1.f );
    glm::mat4 view(1.0f);
    glm::mat4 modelview = view*model; // model-view-projection

    int loc_modelview = glGetUniformLocation(program, "modelview");
...
void create_buffers()
{
  ...
  /* // deprecated fix to force glBufferData allocating device memory
  glBufferData( GL_ARRAY_BUFFER,
                MESH_COUNT * 8 * sizeof(GLfloat),
                NULL,
                GL_DYNAMIC_DRAW );
  // get mapped pointer to buffer for initializing data
  // (FPS goes down if using glBufferData() directly for data upload)
  void* d_data = (void*)glMapBuffer(GL_ARRAY_BUFFER, GL_READ_WRITE);

  memcpy(d_data, (const void*)h_data, MESH_COUNT * 8 * sizeof(GLfloat));
  ...
  for(unsigned int i=0; i<MESH_COUNT*4; i+=4)
  {
    ...
    h_state[i+3] = 75.0f + ((GLfloat)rand()/RAND_MAX) * 25.0f; 
  }
