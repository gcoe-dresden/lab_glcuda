...
//  https://rauwendaal.net/2011/12/02/writing-to-3d-opengl-textures-in-cuda-4-1-with-3d-surface-writes/
...
cudaGraphicsResource* cuda_tbo_resource = nullptr; // texture object for cuda interop
GLfloat* cuda_tbo_buffer = nullptr;
...
static constexpr int MESH_WIDTH = 128;
static constexpr int MESH_HEIGHT = 128;
...
int main(int argc, char** argv)
{
  ...
  */
  ...
  std::string vertex_shader_text   = readFile("shaders/terrain-05.vert");
  std::string fragment_shader_text = readFile("shaders/terrain-05.frag");
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    GLint loc_tex = glGetUniformLocation(program, "texImage");
    glUniform1i(loc_tex, 1); // texture unit 1 to "texImage"
...
void run_cuda()
{
  ...
  cudaArray *cuda_tbo_array;
  CHECK_CUDA(cudaGraphicsMapResources(1, &cuda_tbo_resource, 0));
  CHECK_CUDA(cudaGraphicsSubResourceGetMappedArray(&cuda_tbo_array, cuda_tbo_resource,0,0));

  int size_tex_data = sizeof(GLfloat) * MESH_COUNT * 4;
  CHECK_CUDA(cudaMemcpyToArray(cuda_tbo_array, 0, 0, cuda_tbo_buffer, size_tex_data, cudaMemcpyDeviceToDevice));


  CHECK_CUDA(cudaGraphicsUnmapResources(1, &cuda_tbo_resource, 0));
}
...
void create_buffers()
{
  /* OpenGL: 
    ...
    */
  GLfloat *h_data = new GLfloat[10*MESH_COUNT];
  ...
  // initializing tex coord
  for (unsigned int i = 0; i < MESH_HEIGHT; ++i)
  {
    for (unsigned int j = 0; j < MESH_WIDTH; ++j)
    {
      unsigned id = 2*(j+MESH_WIDTH*i) + MESH_COUNT*8;
      h_data[id]   = 1.0f*j/MESH_WIDTH;
      h_data[id+1] = 1.0f*i/MESH_HEIGHT;
    }
  }
  ...
  // tex coords
  glVertexAttribPointer(2,             // layout index for position in shader
                        2,             // number of components
                        GL_FLOAT,      // type
                        GL_FALSE,      // do not normalize values
                        0,             // stride in bytes
                        (GLvoid*) (8*MESH_COUNT*sizeof(GLfloat)) // offset in array for first component
    );
  glEnableVertexAttribArray(2); // enable vertex attribute with layout index 1
  ...
  // ------------------------------------------------------------------------
  // texture buffer and CUDA interop
  // texture generation
  glActiveTexture(GL_TEXTURE1);
  glGenTextures(1, &tbo);
  // initialize the texture to be the given size
  glBindTexture( GL_TEXTURE_2D, tbo );
  glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA32F, MESH_WIDTH, MESH_HEIGHT, 0, GL_RGBA, GL_FLOAT, nullptr );
  ...
  CHECK_CUDA(cudaGraphicsGLRegisterImage(&cuda_tbo_resource,
                                         tbo,
                                         GL_TEXTURE_2D,
...
//                                         |
  ...
  CHECK_CUDA(cudaMalloc(&cuda_tbo_buffer, 4*MESH_COUNT*sizeof(GLfloat)));
...
void cleanup()
{
  ...
  CHECK_CUDA(cudaGraphicsUnregisterResource(cuda_vbo_resource));
  CHECK_CUDA(cudaGraphicsUnregisterResource(cuda_tbo_resource));
  glDeleteVertexArrays(1, &vao);
  glDeleteBuffers(1, &vbo);
  glDeleteBuffers(1, &ebo);
  glDeleteTextures(1, &tbo);
  CHECK_CUDA(cudaFree(cuda_tbo_buffer));
}
